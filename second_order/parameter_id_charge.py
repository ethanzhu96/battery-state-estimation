...
def exp_relax_charge_2rc(t, v_inf, A1, tau1, A2, tau2):
...
def load_data(csv_path):
    ...
    needed = ["Test Time (s)", "Current (A)", "Voltage (V)", "Charge Capacity (Ah)"]
    for col in needed:
    ...
    return df, t, i, v, ccap

def find_main_charge_window(i):
    active = np.where(i > ACTIVE_CURRENT_THRESHOLD)[0]
    if len(active) == 0:
        raise ValueError("Could not find charge period from current.")
    return int(active[0]), int(active[-1])

def coulomb_count_soc_charge(t, i, start_idx, end_idx):
    soc = np.full_like(t, np.nan, dtype=float)
    soc[start_idx] = 0.0

    charged_ah = 0.0
    charged_history = np.zeros_like(t, dtype=float)
    for k in range(start_idx, end_idx):
        dt = max(t[k + 1] - t[k], 0.0)
        ik = max(i[k], 0.0)
        charged_ah += ik * dt / 3600.0
        charged_history[k + 1] = charged_ah

    total_ah = charged_history[end_idx]
    if total_ah <= 0:
        raise ValueError("Total charged Ah computed as zero.")

    for k in range(start_idx, end_idx + 1):
        soc[k] = charged_history[k] / total_ah
...
def fit_rest_segment_charge_2rc(t, i, v, seg_start, seg_end):
    t0 = t[seg_start]
    idx = np.where(
        (t >= t0)
        & (t <= t0 + FIT_WINDOW)
    ...
    total_amp = max(vv[0] - v_inf_guess, 1e-5)
    ...
    vv_fit = exp_relax_charge_2rc(tt, v_inf, A1, tau1, A2, tau2)
...
def main():
    ...
    charge_start, charge_end = find_main_charge_window(i)
    soc, total_ah = coulomb_count_soc_charge(t, i, charge_start, charge_end)
    rests = find_rest_segments(t, i, charge_start, charge_end)

    rows = []
    for seg_num, (s, e) in enumerate(rests, start=1):
        result = fit_rest_segment_charge_2rc(t, i, v, s, e)
        if result is None:
            continue
        result["SOC"] = soc[s]
    ...
    if len(params_df) == 0:
        raise ValueError("No valid charge relaxation segments were fitted.")
    ...
    if len(params_df) == 0:
        raise ValueError("All fitted charge rows were filtered out as nonphysical.")

    params_df.to_csv(OUTPUT_DIR / "identified_charge_params_table_2rc.csv", index=False)
    ...
    np.savez(
        ...
        total_charged_ah=total_ah,
        source_csv=str(csv_path.name),
    )

    print(f"Using CSV: {csv_path.name}")
    print(f"Charge window: start={charge_start}, end={charge_end}")
    print(f"Total charged capacity: {total_ah:.6f} Ah")
    print(f"Valid charge 2RC segments kept: {len(params_df)}")
    print("Saved identified_charge_params_table_2rc.csv")
    print("Saved identified_charge_param_fits_2rc.npz")
