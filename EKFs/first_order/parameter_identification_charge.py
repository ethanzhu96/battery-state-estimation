...
def exp_relax_charge_1rc(t, v_inf, a1, tau1):
...
def fit_rest_segment_charge_1rc(t, i, v, seg_start, seg_end):
    t0 = t[seg_start]
    idx = np.where(
        (t >= t0)
        & (t <= t0 + FIT_WINDOW)
    ...
    a1_guess = max(vv[0] - v_inf_guess, 1e-5)
    amp_upper = max(1.0, 5.0 * abs(a1_guess))

    try:
        popt, _ = curve_fit(
            exp_relax_charge_1rc,
            tt,
            vv,
    ...
    vv_fit = exp_relax_charge_1rc(tt, v_inf, a1, tau1)
...
def main():
    ...
    for seg_num, (s, e) in enumerate(rests, start=1):
        result = fit_rest_segment_charge_1rc(t, i, v, s, e)
        if result is None:
            continue
        result["SOC"] = soc[s]
    ...
    if len(params_df) < 2:
        raise ValueError("Need at least two valid charge segments to fit SOC-dependent 1RC parameters.")

    params_df.to_csv(OUTPUT_DIR / "identified_charge_params_table.csv", index=False)
    ...
    print(f"Valid charge 1RC segments kept: {len(params_df)}")
    print("Saved identified_charge_params_table.csv")
    print("Saved identified_charge_param_fits.npz")
